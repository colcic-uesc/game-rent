...
class GamesController {
  async index(req, res) {
    ...
    const gamesWithImageUrl = games.map((game) => {
      ...
      return {
        ...gameJSON,
        capa_jogo_url: `http://localhost:${process.env.PORT || 3000}/files/uploads/${gameJSON.capa_jogo}`,
      };
    });

    return res.json(gamesWithImageUrl);
  }
  ...
  async show(req, res) {
    ...
    return res.json(gameWithImageUrl);
  }
  ...
  async destroy(req, res) {
    ...
    return res.status(200).json({ message: "Jogo desativado com sucesso." });
  }
}
